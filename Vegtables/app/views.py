...
def about_view(request):
     return render(request,'dashboard1.html')     # about in header
...
def Dash_post(request):
    if request.method == 'POST':
        variable = DashForm(request.POST)
        if variable.is_valid():
            variable.save()
            return redirect('/order')
        else:
            print("Form errors:", variable.errors)
    else:
        variable = DashForm()

    return render(request, 'dashboard1.html', {'variable': variable})
